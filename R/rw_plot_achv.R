...
rw_plot_achv <- function(df){
  df %>% 
    ...
    ggplot2::geom_segment(aes(xend=indicator, y=0, yend=achievement), 
                          color= dplyr::case_when(df$achievement < thres_low  ~ c_ubuntu,
                                                  df$achievement < thres_med  ~ c_amazon,
                                                  TRUE                        ~ c_grullo), 
                          size= 2) +
    ggplot2::geom_point(color = dplyr::case_when(df$achievement < thres_low  ~ c_ubuntu,
                                                 df$achievement < thres_med  ~ c_amazon,
                                                 TRUE                        ~ c_grullo), 
                        size = 6) +
